...
한국어 뜻은 `~을 복습하다` 처럼 물결표로 시작하는 경우가 많다. 이 `~` 는 뜻에 속하므로 경계를 한 칸 뒤로 밀어 뜻 쪽에 붙인다.
...
동일 로직을 스크립트로 재현해 13개 케이스를 확인했다 (하이픈 단어, 쉼표 포함 뜻, 물결표, 문장, 마크다운 불릿, 탭, em dash). 전부 의도대로 갈렸다.
